...
- **Bundler**: Parcel 2 (`bun run dev` for dev server, `bun run build` for production)
...
```bash
bun run dev                 # start dev server (watches all *.html)
bun run build               # production build → dist/
bun run preview             # serve dist/ locally to spot-check the production build
bun run generate:og         # regenerate src/og-image.png from src/og-image.svg
bun run generate:favicon    # regenerate src/favicon*.png from tools.json emojis
bun run generate:index      # rewrite the tool-grid sentinel in src/index.html from src/tools.json
bun run generate:sections   # rewrite FAQ / More tools / JSON-LD blocks in src/*.html from src/tools.json
bun run generate:readme     # rewrite the tools table in README.md from src/tools.json
```
