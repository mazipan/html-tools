  ...
  pages.forEach((pg, idx) => {
    ...
    tile.innerHTML = `
      ...
      <div class="pg-actions">
        <button class="btn-icon ghost pg-rotate-btn" title="Rotate 90° clockwise" aria-label="Rotate page ${idx + 1} clockwise">↻</button>
        <button class="btn-icon danger pg-delete-btn" title="Delete this page" aria-label="Delete page ${idx + 1}"${pages.length <= 1 ? ' disabled' : ''}>✕</button>
      </div>
      <div class="pg-arrow-btns">
        <button class="btn-icon ghost pg-up-btn" title="Move up" aria-label="Move page ${idx + 1} up"${idx === 0 ? ' disabled' : ''}>↑</button>
        <button class="btn-icon ghost pg-down-btn" title="Move down" aria-label="Move page ${idx + 1} down"${idx === pages.length - 1 ? ' disabled' : ''}>↓</button>
      </div>`;
