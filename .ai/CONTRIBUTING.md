...
2. Add an entry for the tool in `src/tools.json` (`slug`, `name`, `icon`, `category`, `card`, `description`, `faqs`). `card` is a short one-liner shown on the index card; `description` is the full SEO text. Then run `bun run generate:sections` to write the FAQ block, "More tools" cross-link block, and JSON-LD structured data into `src/<slug>.html` (and refresh every other tool's "More tools" list so the new tool shows up there too). Run `bun run generate:index` to add the tool card to `index.html`. Also run `bun run generate:favicon` to rasterize the tool's emoji into `src/favicon-<slug>.png` and commit it.
...
7. Run `bun run generate:index` to insert the tool card into `index.html` automatically. Run `bun run generate:readme` to regenerate the tools table in `README.md`. Both scripts read `tools.json` and replace sentinel-wrapped regions in place. Update the "Current tools" list in `.ai/TOOLS.md`.
...
- `scripts/generate-index.mjs` (`bun run generate:index`) — rewrites the `<!-- BEGIN:tool-grid --> … <!-- END:tool-grid -->` sentinel in `src/index.html` with category sections built from `tools.json` order. Run this whenever a tool is added, removed, or recategorised.
- `scripts/generate-sections.mjs` (`bun run generate:sections`) — writes three blocks directly into `src/*.html` so they're visible in `bun run dev` and bundle through Parcel like the rest of the markup:
...
**Re-run both `bun run generate:index` and `bun run generate:sections` whenever you edit `src/tools.json`.** Both scripts are idempotent — sentinel-wrapped regions are replaced in place on every run, so re-running can never produce duplicate blocks. The generated regions are committed as source.

## Social card image

The 1200×630 social card lives at `src/og-image.png` and is committed to the repo. The source is `src/og-image.svg`; rerun `bun run generate:og` whenever the SVG changes and commit the regenerated PNG. The build only copies `src/og-image.png` to `dist/` — it does not regenerate it on every build.
...
Rerun `bun run generate:favicon` whenever a tool's icon changes or a new tool is added to `tools.json`, then commit the regenerated PNGs. The generator reads Twemoji SVGs from `node_modules/@twemoji/svg` (no network at run time) and renders them with Resvg.

When adding a new tool, after registering it in `tools.json` and running `bun run generate:favicon`, point the per-page `<link rel="icon">` at the new `favicon-<slug>.png`.
