...
4. Include the shared site header at the top of `<body>` with a `<nav aria-label="Breadcrumb">` linking back to `index.html` — copy the header block from an existing tool. Mark the current page span with `aria-current="page"`. See "Semantic landmarks" in `UI_CONVENTIONS.md`.
...
7. Run `npm run generate:index` to insert the tool card into `index.html` automatically (the script reads `tools.json` and regenerates the sentinel-wrapped grid). Update the "Current tools" list in `TOOLS.md` and add a row for the new tool in the tools table in `README.md`.
