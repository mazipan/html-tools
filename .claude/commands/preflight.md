...
4. **Build**: `bun run build`. If it fails, stop and surface the error verbatim — don't try to fix it without the user's input.

5. **Optional spot check**: if the change touches a tool page or shared styles, suggest `bun run preview` and tell the user which URLs to open. Don't start the preview server yourself unless asked.
