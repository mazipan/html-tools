...
1. **Read the issue** with `mcp__github__issue_read` (repo `mazipan/html-tools`, issue `$ARGUMENTS`). Skim the body, labels, and any linked umbrella issue (look for an `Umbrella: #N` line in the body).
...
4. **Update the umbrella issue** if the child issue body contains an `Umbrella: #N` line: flip the marker to `📝 spec'd (issue or PR open)` and link issue #$ARGUMENTS. Don't do this silently — tell the user which umbrella you're touching. If no umbrella is referenced, skip this step.
